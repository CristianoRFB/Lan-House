...
if not defined ADRENALINA_PACKAGE_ROOT if exist "%~dp0..\Adrenalina.slnx" if exist "%~dp0Download-ReleasePackage.ps1" (
    echo Pacote ADMIN nao encontrado. Baixando a versao publicada...
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0Download-ReleasePackage.ps1" -Role ADMIN -DestinationRoot "%~dp0..\artifacts\release"
