...
try {
    ...
    $discoveryRule = Get-NetFirewallRule -DisplayName 'Adrenalina Discovery LAN (Private)' -ErrorAction SilentlyContinue
    Check (($null -ne $rule) -or $uri.Host -in @('127.0.0.1', 'localhost')) 'Regra de firewall da LAN não encontrada.'
    Check (($null -ne $discoveryRule) -or $uri.Host -in @('127.0.0.1', 'localhost')) 'Regra de descoberta LAN não encontrada.'
