...
internal sealed class LauncherForm : Form
{
    ...
    public LauncherForm()
    {
        ...
        var adminAvailable = File.Exists(Path.Combine(AppContext.BaseDirectory, "Adrenalina.Admin.exe"));
        var clientAvailable = File.Exists(Path.Combine(AppContext.BaseDirectory, "Adrenalina.Client.exe"));

        if (adminAvailable)
        {
            var adminButton = CreateRoleButton("ADMIN", "Gerenciar a Lan House", Color.FromArgb(57, 96, 168));
            adminButton.Location = new Point(clientAvailable ? 42 : 185, 132);
        ...
        if (clientAvailable)
        {
            ...
            clientButton.Location = new Point(adminAvailable ? 318 : 185, 132);
            clientButton.Click += (_, _) => Launch("Adrenalina.Client.exe");
            Controls.Add(clientButton);
        }

        _status.Text = adminAvailable || clientAvailable
            ? "Selecione uma opção para continuar."
            : "Nenhum aplicativo disponível nesta pasta. Reinstale o pacote correto.";
